...
export default function CashoutPOS() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      fd.append('image', f);
      // Curl equivalent: curl -X POST https://rrimg.chinatownlogistic.com/upload -F "image=@..."
      const res = await fetch('https://rrimg.chinatownlogistic.com/upload', {
        method: 'POST',
        body: fd,
      });
      ...
      // Construct full URL if needed, or if API returns relative.
      // API returns: { url: "/public/uploads/..." }
      const finalUrl = data.url.startsWith('http') ? data.url : `https://rrimg.chinatownlogistic.com${data.url}`;
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className={`w-full p-3 rounded-xl border text-left flex items-center gap-3 ${role === 'finance' ? 'opacity-50 cursor-not-allowed border-[var(--border-color)]' : retiroTipo === 'gasto_operativo' ? 'border-[var(--primary-color)] bg-[var(--primary-color)]/10 shadow-[0_10px_30px_-18px_var(--primary-color)]' : 'border-[var(--border-color)] hover:bg-white/5'}`}
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-[var(--text-secondary-color)]">{role === 'finance' ? 'Deshabilitado para tu rol' : 'Requiere imagen de soporte. Se guarda en historial de gastos.'}</p>
                    </div>
