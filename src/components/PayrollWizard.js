...
export default function PayrollWizard({ isOpen, onClose, employee, config, onConfirm, initialDates }) {
    ...
    }, [isOpen, employee, initialDates, config?.valor_dominical, config?.valor_dominical_s1, config?.valor_dominical_s2, config?.valor_madrugon, config?.porcentaje_salud, config?.porcentaje_pension]);

    // Derived Values
    const paidBase = baseSalary / 2;
    const transport = (config?.auxilio_transporte || 0) / 2;
    ...
    const handleUpdateBaseSalary = async () => {
        ...
        try {
            const res = await apiFetch(`/api/nomina/employees/${employee.id}/salary`, {
                ...
                body: JSON.stringify({ base_salary: Number(tempBaseSalary) })
            });

            if (res.ok) {
                setBaseSalary(Number(tempBaseSalary));
                setIsEditingBase(false);
                notify({ type: 'success', message: 'Salario base actualizado correctamente' });
            } else {
                const errorData = await res.json();
                notify({ type: 'error', message: errorData.error || 'Error al actualizar el salario' });
            }
        } catch (e) {
            console.error(e);
    ...
    const totalCalculated = useMemo(() => {
        ...
        return paidBase + transport + sundaysVal + madrugonesVal + incomeAdj - health - pension - advance - dedAdj;
    }, [paidBase, transport, sundaysQty, sundayValue, madrugonesQty, madrugonValue, health, pension, advance, adjustments]);
    ...
    const handleConfirm = async () => {
        ...
        try {
            const payload = {
                ...
                sundays_qty: sundaysQty,
                madrugones_qty: madrugonesQty,
                advance: advance,
                includes_security: includesSecurity,
                notes: JSON.stringify(adjustments),
                aditions: JSON.stringify(adjustments.filter(a => a.type === 'income')),
                deductions: JSON.stringify(adjustments.filter(a => a.type === 'deduction')),
            };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="bg-[var(--card-color)] w-full max-w-4xl max-h-[90vh] rounded-2xl border border-[var(--border-color)] shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="p-6 border-b border-[var(--border-color)] flex justify-between items-center bg-[#111]">
                    <div>
                        <h2 className="text-xl font-bold">Generar Pago: {employee?.name}</h2>
                        <div className="text-xs text-[var(--text-secondary-color)] mt-1 flex gap-2">
                            <span>Paso {currentStep} de {STEPS.length}: {STEPS[currentStep - 1].title}</span>
                        </div>
                    </div>
                    {/* Stepper visual */}
                    ...
                    {currentStep === 2 && (
                        <div className="space-y-10 max-w-lg mx-auto py-4">
                            <div className="text-center space-y-2">
                                <h3 className="text-lg font-bold text-[var(--text-secondary-color)] uppercase tracking-widest">Salario Base Actual</h3>
                                ...
                                {isEditingBase ? (
                                    <div className="flex flex-col items-center gap-4 animate-in fade-in zoom-in-95 duration-200">
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => { setIsEditingBase(false); setTempBaseSalary(baseSalary); }}
                            ...
                            <div className="bg-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/5 shadow-inner relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex flex-col items-center text-center">
                                    <span className="text-xs font-bold text-[var(--text-secondary-color)] uppercase tracking-widest mb-1">Pago Quincenal Proyectado</span>
                                    <div className="text-3xl font-bold text-[var(--success-color)] mb-2">
                                        {formatCLP(paidBase)}
                                    </div>
                                    <p className="text-[10px] text-[var(--text-secondary-color)] max-w-[240px] leading-relaxed">
                                    ...
                                    {isEditingSundayValue ? (
                                        <div className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2 duration-200">
                                            ...
                                            <button
                                                ...
                                                disabled={loading}
                                                className="bg-[var(--success-color)] text-black p-1.5 rounded-lg hover:brightness-110 active:scale-95 transition-all"
                                            >
                            ...
                            <div className="space-y-4 max-w-lg mx-auto bg-[var(--background-color)] p-6 rounded-2xl border border-[var(--border-color)]">
                                ...
                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-[var(--text-secondary-color)]">Salario Base Quincenal (50%)</span>
...

                    {
                        currentStep < 8 ? (
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="px-8 py-2 bg-[var(--success-color)] text-black rounded-lg font-bold hover:brightness-110 shadow-lg shadow-green-500/20 active:scale-95 transition-all"
                            >
